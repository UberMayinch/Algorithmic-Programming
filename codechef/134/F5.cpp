...
void add_neighbours(pair<pair<pair<int, int>, int>, int> ele, int n, int m, queue<pair<pair<pair<int, int>, int>, int>>& q, vector<bool>& done, int& ans) {
    ...
    if (i + 1 < n) {
        ...
        else q.push(mp(mp(mp(i + 1, j + 1), turns + 1), ele.second));
        }
        if (j - 1 >= 0) {
            if(grid[i+1][j-1] == 0){
   q.push(mp(mp(mp(i + 1, j - 1), turns), ele.second));
